...
// --- Safe depth-limited serialization for hybrid read mode ---
...
function readSerializeOptions(payload: Record<string, unknown>, defaultDepth: number): SerializeOptions {
  return {
    ...
    maxTextLength: typeof payload.maxTextLength === "number" ? Math.max(0, Math.min(10_000, Math.floor(payload.maxTextLength))) : 500,
    compact: payload.compact !== false,
  };
}

function compactPaints(paints: Paint[]) {
...
function serializeSceneNode(node: SceneNode, options: SerializeOptions): SerializedNode {
  ...
  // Visual style
  if ("fills" in node) {
    var fillsVal = node.fills as Paint[] | typeof figma.mixed;
    if (fillsVal !== figma.mixed) {
      result.fills = (options.compact ? compactPaints(fillsVal) : fillsVal.slice()) as Paint[];
    }
  }
  if ("strokes" in node) {
    var strokesVal = node.strokes as Paint[] | typeof figma.mixed;
    if (strokesVal !== figma.mixed && strokesVal.length > 0) {
      result.strokes = (options.compact ? compactPaints(strokesVal) : strokesVal.slice()) as Paint[];
    }
  }
  if ("strokeWeight" in node) {
    var sw = node.strokeWeight as number | typeof figma.mixed;
    if (sw !== figma.mixed) result.strokeWeight = sw;
  }
  if ("cornerRadius" in node) {
    var cr = node.cornerRadius as number | typeof figma.mixed;
    if (cr !== figma.mixed && cr > 0) result.cornerRadius = cr;
  }
  ...
  if (node.type === "TEXT") {
    ...
    var fs = textNode.fontSize as number | typeof figma.mixed;
    if (fs !== figma.mixed) result.fontSize = fs;
    var fn = textNode.fontName as FontName | typeof figma.mixed;
    if (fn !== figma.mixed) result.fontName = Object.assign({}, fn);
    var lh = textNode.lineHeight as LineHeight | typeof figma.mixed;
    if (lh !== figma.mixed) result.lineHeight = lh;
    var ls = textNode.letterSpacing as LetterSpacing | typeof figma.mixed;
    if (ls !== figma.mixed) result.letterSpacing = ls;
  ...
  if ("layoutMode" in node) {
    var layoutNode = node as FrameNode | ComponentNode | InstanceNode;
    if (layoutNode.layoutMode !== "NONE") {
      ...
      if ("layoutWrap" in layoutNode) result.layoutWrap = layoutNode.layoutWrap;
    }
  }
  ...
  try {
    if ("variantProperties" in node && node.variantProperties) {
      result.variantProperties = Object.assign({}, node.variantProperties as Record<string, string>);
    }
  } catch (_vpError) {
  ...
  try {
    if (node.type === "COMPONENT_SET" && "componentPropertyDefinitions" in node) {
      result.componentPropertyDefinitions = Object.assign({}, node.componentPropertyDefinitions as Record<string, unknown>);
  ...
  try {
    if ("componentProperties" in node && node.componentProperties) {
      result.componentProperties = Object.assign({}, node.componentProperties as Record<string, unknown>);
    }
  } catch (_cpError) {
  ...
  try {
    if ("componentPropertyReferences" in node && node.componentPropertyReferences) {
      result.componentPropertyReferences = Object.assign({}, node.componentPropertyReferences as Record<string, string>);
    }
  } catch (_refError) {
  ...
  if ("boundVariables" in node && node.boundVariables) {
    result.boundVariables = Object.assign({}, node.boundVariables as Record<string, unknown>);
  }
  if ("explicitVariableModes" in node && node.explicitVariableModes) {
    result.explicitVariableModes = Object.assign({}, node.explicitVariableModes as Record<string, string>);
  }

  // Recursively serialize children only when depth > 0
  if (options.depth > 0 && "children" in node) {
    var parentWithChildren = node as FrameNode | GroupNode | ComponentNode | InstanceNode | PageNode;
    var childOptions: SerializeOptions = {
    ...
    for (var i = 0; i < parentWithChildren.children.length; i += 1) {
      ...
      if (!options.includeInvisible && !child.visible) continue;
...
function applyLayoutChildProps(node: SceneNode, payload: Record<string, unknown>) {
  if (!("layoutAlign" in node)) return;
  var child = node as FrameNode | ComponentNode | InstanceNode;
  if (typeof payload.layoutAlign === "string") child.layoutAlign = payload.layoutAlign as "MIN" | "CENTER" | "MAX" | "STRETCH" | "INHERIT";
  if (typeof payload.layoutGrow === "number") child.layoutGrow = payload.layoutGrow;
  if (typeof payload.layoutPositioning === "string") child.layoutPositioning = payload.layoutPositioning as "AUTO" | "ABSOLUTE";
  if (typeof payload.minWidth === "number") child.minWidth = payload.minWidth;
  if (typeof payload.maxWidth === "number") child.maxWidth = payload.maxWidth;
  if (typeof payload.minHeight === "number") child.minHeight = payload.minHeight;
  if (typeof payload.maxHeight === "number") child.maxHeight = payload.maxHeight;
}
...
async function createPageFrame(parent: BaseNode & ChildrenMixin, framePayload: Record<string, unknown>) {
...
async function getFrameParent(parentId: unknown) {
  if (typeof parentId !== "string") throw new Error("parentId is required");
  const parent = await figma.getNodeByIdAsync(parentId);
  if (!parent || !("appendChild" in parent)) throw new Error("Parent node not found or cannot contain children");
  return parent as BaseNode & ChildrenMixin;
}

function templatePageNames(payload: Record<string, unknown>) {
  if (Array.isArray(payload.pages) && payload.pages.every((page) => typeof page === "string")) return payload.pages as string[];
  if (payload.template === "portfolio-site") return ["Home", "Work", "Project Detail", "About", "Contact"];
...
function getPropertyDisplayName(key: string, definition: Record<string, unknown>) {
  var defName = typeof (definition as unknown as Record<string, unknown>).name === "string" ? (definition as unknown as Record<string, unknown>).name as string : undefined;
  if (defName) return defName;
  return key.split("#")[0];
}

function resolveComponentPropertyKey(
  definitions: Record<string, ComponentPropertyDefinitions[string]>,
  requested: string,
): string {
  // Exact match
  ...
  for (var ki = 0; ki < keys.length; ki += 1) {
    ...
    if (
      normalizePropertyName(key) === wanted ||
 ...
 */
function findLayersByName(container: BaseNode, layerName: string, matchMode: "exact" | "contains"): SceneNode[] {
  ...
  function search(node: BaseNode) {
    ...
    if ("children" in node) {
      var children = (node as FrameNode | GroupNode | ComponentNode | InstanceNode | PageNode | ComponentSetNode).children;
 ...
 */
async function resolveComponentByName(name: string): Promise<ComponentNode> {
  var allNodes = figma.root.findAll(function(n) { return n.type === "COMPONENT"; }) as ComponentNode[];
  var matches: ComponentNode[] = [];
...
async function createOrUpdatePaintStyle(payload: Record<string, unknown>) {
  if (typeof payload.name !== "string") throw new Error("name is required");
  if (!Array.isArray(payload.paints)) throw new Error("paints is required");
  ...
  if (existing) {
    if (!upsert) throw new Error("Paint style '" + payload.name + "' already exists. Use upsert:true to update.");
    existing.paints = normalizePaints(payload.paints as Paint[]);
    if (typeof payload.description === "string") existing.description = payload.description;
    var result2: Record<string, unknown> = { id: existing.id, name: existing.name, operation: "updated" };
    if (match.duplicates.length > 0) result2.duplicateIds = match.duplicates;
    return result2;
  }
  ...
  style.paints = normalizePaints(payload.paints as Paint[]);
  return { id: style.id, name: style.name, operation: "created" };
}

async function createOrUpdateTextStyle(payload: Record<string, unknown>) {
  if (typeof payload.name !== "string") throw new Error("name is required");
  if (typeof payload.fontFamily !== "string") throw new Error("fontFamily is required");
  if (typeof payload.fontStyle !== "string") throw new Error("fontStyle is required");
  if (typeof payload.fontSize !== "number") throw new Error("fontSize is required");
  ...
  if (existing) {
    if (!upsert) throw new Error("Text style '" + payload.name + "' already exists. Use upsert:true to update.");
    existing.fontName = fontName;
    existing.fontSize = payload.fontSize;
    if (payload.lineHeight && typeof payload.lineHeight === "object") existing.lineHeight = payload.lineHeight as LineHeight;
    if (payload.letterSpacing && typeof payload.letterSpacing === "object") existing.letterSpacing = payload.letterSpacing as LetterSpacing;
    if (typeof payload.paragraphSpacing === "number") existing.paragraphSpacing = payload.paragraphSpacing;
    if (typeof payload.paragraphIndent === "number") existing.paragraphIndent = payload.paragraphIndent;
    if (typeof payload.description === "string") existing.description = payload.description;
    var tResult: Record<string, unknown> = { id: existing.id, name: existing.name, operation: "updated" };
    if (tMatch.duplicates.length > 0) tResult.duplicateIds = tMatch.duplicates;
    return tResult;
  }
  ...
  if (payload.lineHeight && typeof payload.lineHeight === "object") tstyle.lineHeight = payload.lineHeight as LineHeight;
  if (payload.letterSpacing && typeof payload.letterSpacing === "object") tstyle.letterSpacing = payload.letterSpacing as LetterSpacing;
  if (typeof payload.paragraphSpacing === "number") tstyle.paragraphSpacing = payload.paragraphSpacing;
  if (typeof payload.paragraphIndent === "number") tstyle.paragraphIndent = payload.paragraphIndent;
...
async function createOrUpdateEffectStyle(payload: Record<string, unknown>) {
  if (typeof payload.name !== "string") throw new Error("name is required");
  if (!Array.isArray(payload.effects)) throw new Error("effects is required");
  ...
  if (existing) {
    if (!upsert) throw new Error("Effect style '" + payload.name + "' already exists. Use upsert:true to update.");
    existing.effects = normalizeEffects(payload.effects as Effect[]);
    if (typeof payload.description === "string") existing.description = payload.description;
    var eResult: Record<string, unknown> = { id: existing.id, name: existing.name, operation: "updated" };
    if (eMatch.duplicates.length > 0) eResult.duplicateIds = eMatch.duplicates;
    return eResult;
  }
  ...
  style.name = payload.name;
  style.description = typeof payload.description === "string" ? payload.description : "";
  style.effects = normalizeEffects(payload.effects as Effect[]);
  return { id: style.id, name: style.name, operation: "created" };
}

function findStylesByName<T extends { name: string; id: string }>(styles: T[], name: string): { first: T | undefined; duplicates: string[] } {
  var first: T | undefined;
  var duplicates: string[] = [];
  for (var i = 0; i < styles.length; i += 1) {
    if (styles[i].name === name) {
      if (!first) { first = styles[i]; }
      else { duplicates.push(styles[i].id); }
    }
  }
...
async function handleCommand(command: BridgeCommand) {
  ...
  if (command.type === "createFrame" || command.type === "createAutoLayoutFrame") {
    const node = await createSceneNodeSafely(
      ...
      payload,
      "MCP Frame",
      command.type === "createAutoLayoutFrame"
  ...
  if (command.type === "createPolygon") {
    const node = await createSceneNodeSafely(
      ...
      payload,
      "MCP Polygon",
      (polygonNode) => { if (typeof payload.pointCount === "number") polygonNode.pointCount = payload.pointCount; },
    );
    figma.currentPage.selection = [node];
    figma.viewport.scrollAndZoomIntoView([node]);
    return serializeNode(node);
  ...
  if (command.type === "createStar") {
    const node = await createSceneNodeSafely(
      ...
      "MCP Star",
      (starNode) => {
        if (typeof payload.pointCount === "number") starNode.pointCount = payload.pointCount;
        if (typeof payload.innerRadius === "number") starNode.innerRadius = payload.innerRadius;
      },
    );
    figma.currentPage.selection = [node];
    figma.viewport.scrollAndZoomIntoView([node]);
  ...
  if (command.type === "createVector") {
    const node = await createSceneNodeSafely(
      ...
      "MCP Vector",
      (vectorNode) => {
        if (Array.isArray(payload.vectorPaths)) vectorNode.vectorPaths = payload.vectorPaths as VectorPath[];
      },
    );
    figma.currentPage.selection = [node];
    figma.viewport.scrollAndZoomIntoView([node]);
  ...
  if (command.type === "createComponent") {
    const node = await createSceneNodeSafely(
      ...
      payload,
      "MCP Component",
      (componentNode) => { applyAutoLayout(componentNode, payload); },
    );
    figma.currentPage.selection = [node];
    figma.viewport.scrollAndZoomIntoView([node]);
    return serializeNode(node);
  ...
  if (command.type === "createImageRectangle") {
    ...
    try {
      ...
      try { node.remove(); } catch (_cleanupError) { /* ignore cleanup errors */ }
      throw error;
    }
  }
  ...
  if (command.type === "createPageFrames") {
    ...
    if (!Array.isArray(payload.frames)) throw new Error("frames is required");
    const nodes: SceneNode[] = [];
    for (const framePayload of payload.frames) {
      if (!framePayload || typeof framePayload !== "object") continue;
      nodes.push(await createPageFrame(parent, framePayload as Record<string, unknown>));
    }
  ...
  if (command.type === "getNode") {
    ...
    if (!targetNode || targetNode.type === "DOCUMENT" || targetNode.type === "PAGE") throw new Error("Scene node not found");
  ...
  if (command.type === "getNodeTree") {
    ...
    if (!treeNode || treeNode.type === "DOCUMENT" || treeNode.type === "PAGE") throw new Error("Scene node not found");
  ...
  if (command.type === "createInstanceSmart") {
    ...
    var smartInstance: InstanceNode;
    ...
    try {
      if (typeof payload.name === "string") smartInstance.name = payload.name;
      ...
      var requestedProps = payload.properties && typeof payload.properties === "object"
        ? payload.properties as Record<string, string | boolean>
        : {};

      // Get definitions from source
      var sourceDefs: Record<string, ComponentPropertyDefinitions[string]> = {};
      if (smartSrcNode.type === "COMPONENT_SET") {
        try { sourceDefs = smartSrcNode.componentPropertyDefinitions as Record<string, ComponentPropertyDefinitions[string]>; } catch (_e) { /* ignore */ }
      } else if (smartSrcNode.type === "COMPONENT") {
        try { sourceDefs = smartSrcNode.componentPropertyDefinitions as Record<string, ComponentPropertyDefinitions[string]>; } catch (_e) { /* ignore */ }
      }

      var resolvedProperties: Record<string, string | boolean> = {};
      var requestedDisplayNames: string[] = Object.keys(requestedProps);
      ...
      for (var rpi = 0; rpi < requestedDisplayNames.length; rpi += 1) {
        ...
        if (Object.keys(sourceDefs).length > 0) {
          try {
            ...
            if (smartSrcNode.type === "COMPONENT_SET") {
              var defVariant = (smartSrcNode as ComponentSetNode).defaultVariant;
              if (defVariant.variantProperties && propName in (defVariant.variantProperties as Record<string, string>)) {
                isVariantProp = true;
              }
            } else if (smartSrcNode.type === "COMPONENT" && smartSrcNode.variantProperties) {
              if (propName in (smartSrcNode.variantProperties as Record<string, string>)) {
                isVariantProp = true;
              }
            }
            if (isVariantProp) {
      ...
      var componentProperties: Record<string, unknown> | undefined;
      var verifyErrors: string[] = [];
      if (doVerify && Object.keys(resolvedProperties).length > 0) {
        try { componentProperties = Object.assign({}, smartInstance.componentProperties as unknown as Record<string, unknown>); } catch (_e) { /* ignore */ }
        ...
        for (var vki = 0; vki < vKeys.length; vki += 1) {
          ...
          var vActual: string | boolean | undefined;
          if (rawEntry && typeof rawEntry === "object" && "value" in rawEntry) {
            vActual = (rawEntry as { value: string | boolean }).value;
          } else {
            vActual = rawEntry as string | boolean | undefined;
          }
              ...
              try {
                ...
                for (var tli = 0; tli < textLayers.length; tli += 1) {
                  if (textLayers[tli].type === "TEXT") {
                    var chars = (textLayers[tli] as TextNode).characters;
                    if (chars === vExpected) break; // found match
                  }
                }
        ...
        if (smartInstance.variantProperties) {
          var vp = smartInstance.variantProperties as Record<string, string>;
          ...
          for (var vpi = 0; vpi < vpKeys.length; vpi += 1) {
            ...
            if (vpKey in (requestedProps as Record<string, unknown> || {})) {
      ...
      try { smartInstance.remove(); } catch (_cleanupError) { /* ignore cleanup errors */ }
      throw error;
    }
  }
  ...
  if (command.type === "auditComponentProperties") {
    ...
    var definitions: Record<string, ComponentPropertyDefinitions[string]> = {};
    try { definitions = Object.assign({}, csNode.componentPropertyDefinitions as Record<string, ComponentPropertyDefinitions[string]>); } catch (_e) { /* ignore */ }

    // Audit each property
    var auditProperties: Record<string, unknown>[] = [];
    var issues: string[] = [];
    var warnings: string[] = [];
    ...
    for (var dki = 0; dki < defKeys.length; dki += 1) {
      ...
      var propInfo: Record<string, unknown> = {
        key: defKey,
      ...
      var references: Record<string, unknown>[] = [];
      if (isComponentSet && "children" in auditNode) {
        var acs = auditNode as ComponentSetNode;
        ...
        var sc = auditNode as FrameNode | ComponentNode;
    ...
    var probeInstance: InstanceNode | null = null;
    var probeResults: Record<string, unknown>[] = [];
    if (payload.createProbeInstance === true) {
      try {
        if (auditNode.type === "COMPONENT") {
          probeInstance = (auditNode as ComponentNode).createInstance();
        } else {
          probeInstance = (auditNode as ComponentSetNode).defaultVariant.createInstance();
        }
        ...
        for (var ppi = 0; ppi < auditProperties.length; ppi += 1) {
          ...
          if (ap.type === "VARIANT") continue;
          try {
            var probeKey = ap.key as string;
            var probeVal: string | boolean;

            if (ap.type === "BOOLEAN") {
              probeVal = true;
            } else if (ap.type === "INSTANCE_SWAP") {
              // Use defaultValue or preferredValues to find a valid swap target
              var swapDef = definitions[probeKey] as Record<string, unknown> | undefined;
              var swapDefault = swapDef && typeof swapDef.defaultValue === "string" ? swapDef.defaultValue as string : undefined;
              var preferredList = swapDef && Array.isArray(swapDef.preferredValues)
                ? swapDef.preferredValues as { type?: string; key?: string }[]
                : undefined;
              ...
              if (swapDefault) {
                ...
                if (swapKey) {
                  var matched = figma.root.findAll(function(n) { return "key" in n && n.key === swapKey; }) as SceneNode[];
          ...
          var _probeObj: Record<string, string | boolean>;
        }
    ...
    if (payload.cleanupProbe !== false && probeInstance) {
      try { probeInstance.remove(); } catch (_cleanupError) { /* ignore cleanup errors */ }
      probeInstance = null;
    }
  ...
  if (command.type === "bindComponentProperty") {
    if (typeof payload.componentId !== "string") throw new Error("componentId is required");
    if (typeof payload.propertyName !== "string") throw new Error("propertyName is required");
    if (typeof payload.layerName !== "string") throw new Error("layerName is required");
    if (typeof payload.field !== "string") throw new Error("field is required");
    ...
    var bindDefs: Record<string, ComponentPropertyDefinitions[string]> = {};
    try { bindDefs = Object.assign({}, bindCompNode.componentPropertyDefinitions as Record<string, ComponentPropertyDefinitions[string]>); } catch (_e) { bindDefs = {}; }
    ...
    var field = payload.field as string;

    if (bindCompNode.type === "COMPONENT_SET") {
      // Two-phase binding: scan all variants first, then write atomically
      var bcs = bindCompNode as ComponentSetNode;
      var bindTargets: { variant: ComponentNode; layer: SceneNode; oldRefs: Record<string, string> | null }[] = [];
      var missedVariants: string[] = [];

      // Phase 1: scan all variants, validate each target
      for (var bvi = 0; bvi < bcs.children.length; bvi += 1) {
        var bvc = bcs.children[bvi];
        if (bvc.type !== "COMPONENT") continue;
        var layers = findLayersByName(bvc, payload.layerName, matchMode as "exact" | "contains");
        if (layers.length === 0) {
        ...
        var oldRefs: Record<string, string> | null = null;
        try { oldRefs = Object.assign({}, layers[0].componentPropertyReferences as Record<string, string> || {}); } catch (_e) { oldRefs = {}; }
      ...
      for (var bti = 0; bti < bindTargets.length; bti += 1) {
        ...
        try {
          ...
          for (var rbi = bti - 1; rbi >= 0; rbi -= 1) {
            try { bindTargets[rbi].layer.componentPropertyReferences = bindTargets[rbi].oldRefs || {}; } catch (_rollbackError) { /* ignore */ }
          }
          throw _writeError;
        }
      }
      ...
      // Single component
      var bcomp = bindCompNode as ComponentNode;
      var blayers = findLayersByName(bcomp, payload.layerName, matchMode as "exact" | "contains");
      if (blayers.length === 0) {
      ...
      var brefs: Record<string, string> = {};
  ...
  if (command.type === "createComponentWithProperties") {
    ...
    try {
      ...
      if (payload.layoutMode) applyAutoLayout(cwpNode, payload);

      // Create child layers
      var cwpLayers = Array.isArray(payload.layers) ? payload.layers as Record<string, unknown>[] : [];
      for (var cli = 0; cli < cwpLayers.length; cli += 1) {
        ...
        var layerNode: SceneNode;

        if (layerType === "TEXT") {
          layerNode = figma.createText();
          layerNode.name = typeof layerSpec.name === "string" ? layerSpec.name : "Text";
          await applyTextStyle(layerNode as TextNode, layerSpec);
          if (typeof layerSpec.text === "string") (layerNode as TextNode).characters = layerSpec.text;
        ...
        if (layerSpec.layoutMode && "layoutMode" in layerNode) applyAutoLayout(layerNode, layerSpec);
        cwpNode.appendChild(layerNode);
        createdLayers.push(layerNode);
      }

      // Add component properties
      var cwpProperties = Array.isArray(payload.properties) ? payload.properties as Record<string, unknown>[] : [];
      var cwpPropDefs: Record<string, ComponentPropertyDefinitions[string]> = {};
      for (var cpi = 0; cpi < cwpProperties.length; cpi += 1) {
        ...
        if (propType === "VARIANT") continue; // Variants are created via combineAsVariants

        var defVal: string | boolean = typeof propSpec.defaultValue === "string" || typeof propSpec.defaultValue === "boolean"
          ? propSpec.defaultValue as string | boolean
          : (propType === "BOOLEAN" ? true : "");
        var propKey = cwpNode.addComponentProperty(propName, propType as ComponentPropertyType, defVal);
        ...
        if (propSpec.bind && typeof propSpec.bind === "object") {
          var bindSpec = propSpec.bind as Record<string, unknown>;
          ...
          if (bindLayerName && bindField) {
            ...
            if (bindLayers.length === 0) {
              throw new Error(
                "Cannot bind property '" + propName + "' to layer '" + bindLayerName +
            ...
            var bindRefs: Record<string, string> = {};
      ...
      if (cwpVerify && Object.keys(cwpPropDefs).length > 0) {
        ...
        try {
          ...
          for (var cdki = 0; cdki < cwpDefKeys.length; cdki += 1) {
            ...
            if (cd.type === "TEXT") probeProps[cdk] = "PROBE";
            else if (cd.type === "BOOLEAN") probeProps[cdk] = false;
          }
          probeInst.setProperties(probeProps);

          // Diff: verify every property actually took effect
          var rawCompProps = probeInst.componentProperties as Record<string, { type: ComponentPropertyType; value: string | boolean }>;
          var actualProps: Record<string, string | boolean> = {};
          ...
          try { probeInst.remove(); } catch (_cleanupError) { /* ignore */ }
        }
      }
      ...
      try { cwpNode.remove(); } catch (_cleanupError) { /* ignore */ }
      throw error;
    }
  }

  // --- Helper for audit: search references in subtree ---
  function searchReferencesInSubtree(
    root: BaseNode,
    propertyType: ComponentPropertyType,
    _definition: ComponentPropertyDefinitions[string],
    propKey: string,
    outReferences: Record<string, unknown>[],
  ) {
    if ("componentPropertyReferences" in root) {
      try {
        var refs = root.componentPropertyReferences as Record<string, string> | null;
        if (refs) {
    ...
    if ("children" in root) {
      var rchildren = (root as FrameNode | GroupNode | ComponentNode | InstanceNode).children;
  ...
  if (command.type === "combineAsVariants") {
    if (!Array.isArray(payload.componentIds) || payload.componentIds.length < 2) throw new Error("At least 2 component IDs are required");
    var components: ComponentNode[] = [];
    for (var ci = 0; ci < payload.componentIds.length; ci += 1) {
      var cnode = await figma.getNodeByIdAsync(payload.componentIds[ci]);
      if (!cnode || cnode.type !== "COMPONENT") throw new Error("Component not found: " + payload.componentIds[ci]);
    ...
    var set = figma.combineAsVariants(components, variantParent as BaseNode & ChildrenMixin, variantIndex);
    if (typeof payload.name === "string") set.name = payload.name;
    return {
      id: set.id,
      name: set.name,
      type: set.type,
  ...
  if (command.type === "addComponentProperty") {
    if (typeof payload.componentId !== "string") throw new Error("componentId is required");
    if (typeof payload.propertyName !== "string") throw new Error("propertyName is required");
    if (typeof payload.propertyType !== "string") throw new Error("propertyType is required");
    var compNode = await figma.getNodeByIdAsync(payload.componentId);
    if (!compNode || (compNode.type !== "COMPONENT" && compNode.type !== "COMPONENT_SET")) throw new Error("Component or ComponentSet not found");
    var propertyType = payload.propertyType as ComponentPropertyType;
    var defVal: string | boolean;
    if (propertyType === "BOOLEAN") {
      ...
      if (typeof payload.defaultValue !== "string") throw new Error("defaultValue string is required for " + propertyType);
  ...
  if (command.type === "editComponentProperty") {
    if (typeof payload.componentId !== "string") throw new Error("componentId is required");
    if (typeof payload.propertyKey !== "string") throw new Error("propertyKey is required");
    var editCompNode = await figma.getNodeByIdAsync(payload.componentId);
    if (!editCompNode || (editCompNode.type !== "COMPONENT" && editCompNode.type !== "COMPONENT_SET")) throw new Error("Component or ComponentSet not found");
    var existing = editCompNode.componentPropertyDefinitions[payload.propertyKey];
    if (!existing) throw new Error("Property not found: " + payload.propertyKey);
    if (typeof payload.newName === "string") editCompNode.editComponentProperty(payload.propertyKey, { name: payload.newName });
    var newDefault: unknown = payload.newDefaultValue;
  ...
  if (command.type === "setComponentPropertyReference") {
    if (typeof payload.nodeId !== "string") throw new Error("nodeId is required");
    if (!payload.references || typeof payload.references !== "object") throw new Error("references is required");
    var refNode = await figma.getNodeByIdAsync(payload.nodeId);
    if (!refNode || refNode.type === "DOCUMENT" || refNode.type === "PAGE") throw new Error("Scene node not found");
    refNode.componentPropertyReferences = Object.assign({}, refNode.componentPropertyReferences || {}, payload.references as Record<string, string>);
  ...
  if (command.type === "getComponentProperties") {
    ...
    if (!gcpNode || (gcpNode.type !== "COMPONENT" && gcpNode.type !== "COMPONENT_SET")) throw new Error("Component or ComponentSet not found");
  ...
  if (command.type === "createInstanceWithOverrides") {
    ...
    if (!iSrcComp || (iSrcComp.type !== "COMPONENT" && iSrcComp.type !== "COMPONENT_SET")) throw new Error("Component not found");
    var iNode: InstanceNode;
    ...
    try {
      if (typeof payload.name === "string") iNode.name = payload.name;
      ...
      if (payload.properties && typeof payload.properties === "object") {
        iNode.setProperties(payload.properties as Record<string, string | boolean>);
      }
    } catch (error) {
      try { iNode.remove(); } catch (_cleanupError) { /* ignore cleanup errors */ }
      throw error;
    }
  ...
  if (command.type === "addVariableMode") {
    if (typeof payload.collectionId !== "string") throw new Error("collectionId is required");
    if (typeof payload.name !== "string") throw new Error("name is required");
    var modeCollection = await figma.variables.getVariableCollectionByIdAsync(payload.collectionId);
    if (!modeCollection) throw new Error("Variable collection not found");
  ...
  if (command.type === "createVariable") {
    if (typeof payload.collectionId !== "string") throw new Error("collectionId is required");
    if (typeof payload.name !== "string") throw new Error("name is required");
    if (typeof payload.resolvedType !== "string") throw new Error("resolvedType is required");
    var varCollection = await figma.variables.getVariableCollectionByIdAsync(payload.collectionId);
    if (!varCollection) throw new Error("Variable collection not found");
    var variable = figma.variables.createVariable(payload.name, varCollection, payload.resolvedType as VariableResolvedDataType);
    if (Array.isArray(payload.scopes)) variable.scopes = payload.scopes as VariableScope[];
  ...
  if (command.type === "setVariableValueForMode") {
    if (typeof payload.variableId !== "string") throw new Error("variableId is required");
    if (typeof payload.modeId !== "string") throw new Error("modeId is required");
    var valVariable = await figma.variables.getVariableByIdAsync(payload.variableId);
    if (!valVariable) throw new Error("Variable not found");
    valVariable.setValueForMode(payload.modeId, payload.value as VariableValue);
  ...
  if (command.type === "bindVariable") {
    if (typeof payload.nodeId !== "string") throw new Error("nodeId is required");
    if (typeof payload.target !== "string") throw new Error("target is required");
    if (typeof payload.field !== "string") throw new Error("field is required");
    if (typeof payload.variableId !== "string") throw new Error("variableId is required");
    var bindNode = await figma.getNodeByIdAsync(payload.nodeId);
    if (!bindNode || bindNode.type === "DOCUMENT" || bindNode.type === "PAGE") throw new Error("Scene node not found");
    var bindVar = await figma.variables.getVariableByIdAsync(payload.variableId);
    if (!bindVar) throw new Error("Variable not found");

    if (payload.target === "nodeField") {
      bindNode.setBoundVariable(payload.field as VariableBindableNodeField, bindVar);
    } else if (payload.target === "fills") {
      if (!("fills" in bindNode) || (bindNode.fills as Paint[] | typeof figma.mixed) === figma.mixed) throw new Error("Node does not have bindable fills");
      var fills = (bindNode.fills as Paint[]).slice();
      var fillIdx = typeof payload.index === "number" ? payload.index : 0;
      if (fillIdx >= fills.length) throw new Error("Fill index out of range");
      var paint = fills[fillIdx];
      if (paint.type !== "SOLID") throw new Error("Only SOLID paint color binding is supported");
      ...
      if (!("strokes" in bindNode) || (bindNode.strokes as Paint[] | typeof figma.mixed) === figma.mixed) throw new Error("Node does not have bindable strokes");
      var strokes = (bindNode.strokes as Paint[]).slice();
      var strokeIdx = typeof payload.index === "number" ? payload.index : 0;
      if (strokeIdx >= strokes.length) throw new Error("Stroke index out of range");
      var spaint = strokes[strokeIdx];
      if (spaint.type !== "SOLID") throw new Error("Only SOLID stroke color binding is supported");
      ...
      if (!("effects" in bindNode)) throw new Error("Node does not have bindable effects");
      var effects = (bindNode.effects as readonly Effect[]).slice();
      var effectIdx = typeof payload.index === "number" ? payload.index : 0;
      if (effectIdx >= effects.length) throw new Error("Effect index out of range");
      effects[effectIdx] = figma.variables.setBoundVariableForEffect(effects[effectIdx], payload.field as VariableBindableEffectField, bindVar);
  ...
  if (command.type === "setExplicitVariableMode") {
    ...
    if (!modeNode || modeNode.type === "DOCUMENT" || modeNode.type === "PAGE") throw new Error("Scene node not found");
    var modeCollection2 = await figma.variables.getVariableCollectionByIdAsync(payload.collectionId);
    if (!modeCollection2) throw new Error("Variable collection not found");
  ...
  if (command.type === "createThemeTokens") {
    if (typeof payload.collectionName !== "string") throw new Error("collectionName is required");
    if (!Array.isArray(payload.modes) || payload.modes.length === 0) throw new Error("modes is required");
    if (!Array.isArray(payload.tokens) || payload.tokens.length === 0) throw new Error("tokens is required");

    var themeColl = figma.variables.createVariableCollection(payload.collectionName);
    // Rename default mode to first mode name
    if (themeColl.modes.length > 0) themeColl.renameMode(themeColl.modes[0].modeId, payload.modes[0]);
    ...
    var tokenMap: Record<string, string> = {};
    for (var ti = 0; ti < payload.tokens.length; ti += 1) {
      var token = payload.tokens[ti];
      if (typeof token.name !== "string" || typeof token.type !== "string") continue;
      var tvar = figma.variables.createVariable(token.name, themeColl, token.type as VariableResolvedDataType);
      if (Array.isArray(token.scopes)) tvar.scopes = token.scopes as VariableScope[];
      if (token.values && typeof token.values === "object") {
        var tvals = token.values as Record<string, unknown>;
        for (var mk = 0; mk < payload.modes.length; mk += 1) {
          ...
          if (modeKey && modeName in tvals) {
            tvar.setValueForMode(modeKey, tvals[modeName] as VariableValue);
          }
        }
      }
  ...
  if (command.type === "batchCreateNodes") {
    ...
    if (!Array.isArray(nodes) || nodes.length === 0) throw new Error("nodes array is required");
    if (nodes.length > 500) throw new Error("Maximum 500 nodes per batch");
    ...
    for (var ni = 0; ni < nodes.length; ni += 1) {
      ...
      if (!nodeSpec || typeof nodeSpec !== "object") throw new Error("Invalid node at index " + ni);
      var tid = nodeSpec.tempId;
      if (typeof tid !== "string" || tid.length === 0) throw new Error("tempId is required at index " + ni);
      if (tempIdSet.has(tid)) throw new Error("Duplicate tempId: " + tid);
      tempIdSet.add(tid);
    ...
    function resolveParent(spec: Record<string, unknown>) {
      if (typeof spec.parentTempId === "string") {
        var pn = idMap.get(spec.parentTempId);
        if (!pn) throw new Error("Parent tempId " + spec.parentTempId + " not resolved");
        return pn as BaseNode & ChildrenMixin;
      }
      if (typeof spec.parentId === "string") {
        return getFrameParent(spec.parentId);
      }
      return Promise.resolve(figma.currentPage as unknown as BaseNode & ChildrenMixin);
    }
    ...
    async function createNodeByType(spec: Record<string, unknown>, parent: BaseNode & ChildrenMixin): Promise<SceneNode> {
      var props = spec.props && typeof spec.props === "object" ? spec.props as Record<string, unknown> : {};
      var nodeType = typeof spec.type === "string" ? spec.type : "";
      var currentNode: SceneNode | null = null;
      ...
      try {
        switch (nodeType) {
          ...
          case "POLYGON": {
            ...
            if (typeof props.pointCount === "number") poly.pointCount = props.pointCount;
          ...
          case "STAR": {
            ...
            if (typeof props.pointCount === "number") star.pointCount = props.pointCount;
            if (typeof props.innerRadius === "number") star.innerRadius = props.innerRadius;
          ...
          case "VECTOR": {
            ...
            if (Array.isArray(props.vectorPaths)) vec.vectorPaths = props.vectorPaths as VectorPath[];
          ...
          case "INSTANCE": {
            if (typeof props.componentId !== "string") throw new Error("componentId is required for INSTANCE");
            var srcComp = figma.getNodeById(props.componentId);
            if (!srcComp || srcComp.type !== "COMPONENT") throw new Error("Component not found: " + props.componentId);
            ...
            if (props.properties && typeof props.properties === "object") {
              inst.setProperties(props.properties as Record<string, string | boolean>);
            }
            return inst;
          }
          case "IMAGE_RECTANGLE": {
            if (typeof props.imageUrl !== "string") throw new Error("imageUrl is required for IMAGE_RECTANGLE");
        ...
        if (currentNode) {
          try { currentNode.remove(); } catch (_cleanupError) { /* ignore cleanup errors */ }
        }
        throw error;
      }
    }

    var idMap = new Map<string, SceneNode>();
    var createdNodes: SceneNode[] = [];
    var rolledBack = false;
    var failedTempId: string | null = null;
    var failedError: string | null = null;
    ...
    try {
      for (var si = 0; si < nodes.length; si += 1) {
        ...
        var specProps = spec.props && typeof spec.props === "object" ? spec.props as Record<string, unknown> : {};
      ...
      var idMapResult: Record<string, string> = {};
      ...
      if (rollbackOnError) {
        ...
        for (var ri = createdNodes.length - 1; ri >= 0; ri -= 1) {
          try { createdNodes[ri].remove(); } catch (_cleanupError) { /* ignore */ }
        }
      }
  ...
  if (command.type === "updateStyle") {
    ...
    if (typeof styleId !== "string") throw new Error("styleId is required");
    if (typeof sType !== "string") throw new Error("styleType is required");

    if (sType === "paint") {
      var psId = styleId as unknown as string;
      var paintStyle = await figma.getLocalPaintStylesAsync().then(function(s) { return s.find(function(ps) { return ps.id === psId; }); });
      if (!paintStyle) throw new Error("Paint style not found: " + psId);
      if (typeof payload.name === "string") paintStyle.name = payload.name;
      if (typeof payload.description === "string") paintStyle.description = payload.description;
      if (Array.isArray(payload.paints)) paintStyle.paints = normalizePaints(payload.paints as Paint[]);
      return serializePaintStyle(paintStyle);
    } else if (sType === "text") {
      var tsId = styleId as unknown as string;
      var textStyle = await figma.getLocalTextStylesAsync().then(function(s) { return s.find(function(ts) { return ts.id === tsId; }); });
      if (!textStyle) throw new Error("Text style not found: " + tsId);
      if (typeof payload.name === "string") textStyle.name = payload.name;
      if (typeof payload.description === "string") textStyle.description = payload.description;
      ...
      if (typeof payload.fontSize === "number") textStyle.fontSize = payload.fontSize;
      if (payload.lineHeight && typeof payload.lineHeight === "object") textStyle.lineHeight = payload.lineHeight as LineHeight;
      if (payload.letterSpacing && typeof payload.letterSpacing === "object") textStyle.letterSpacing = payload.letterSpacing as LetterSpacing;
      if (typeof payload.paragraphSpacing === "number") textStyle.paragraphSpacing = payload.paragraphSpacing;
      if (typeof payload.paragraphIndent === "number") textStyle.paragraphIndent = payload.paragraphIndent;
      return serializeTextStyle(textStyle);
    } else if (sType === "effect") {
      var esId = styleId as unknown as string;
      var effectStyle = await figma.getLocalEffectStylesAsync().then(function(s) { return s.find(function(es) { return es.id === esId; }); });
      if (!effectStyle) throw new Error("Effect style not found: " + esId);
      if (typeof payload.name === "string") effectStyle.name = payload.name;
      if (typeof payload.description === "string") effectStyle.description = payload.description;
      if (Array.isArray(payload.effects)) effectStyle.effects = normalizeEffects(payload.effects as Effect[]);
  ...
  if (command.type === "deleteStyle") {
    if (typeof payload.styleId !== "string") throw new Error("styleId is required");
    if (typeof payload.styleType !== "string") throw new Error("styleType is required");
    ...
    if (payload.styleType === "paint") {
      ...
      if (!dps) throw new Error("Paint style not found: " + payload.styleId);
      dps.remove();
      ...
      if (!dts) throw new Error("Text style not found: " + payload.styleId);
      dts.remove();
      ...
      if (!des) throw new Error("Effect style not found: " + payload.styleId);
      des.remove();
    }
  ...
  if (command.type === "bindStyle") {
    ...
    if (!bindNode2 || bindNode2.type === "DOCUMENT" || bindNode2.type === "PAGE") throw new Error("Scene node not found");

    var target = payload.target as string;
    if (target === "fill") {
      if (!("setFillStyleIdAsync" in bindNode2)) throw new Error("Node does not support fill styles");
      await (bindNode2 as FrameNode | RectangleNode).setFillStyleIdAsync(payload.styleId);
    } else if (target === "stroke") {
      if (!("setStrokeStyleIdAsync" in bindNode2)) throw new Error("Node does not support stroke styles");
      await (bindNode2 as FrameNode | RectangleNode).setStrokeStyleIdAsync(payload.styleId);
    } else if (target === "text") {
      if (bindNode2.type !== "TEXT") throw new Error("Text style target requires TEXT node");
      await (bindNode2 as TextNode).setTextStyleIdAsync(payload.styleId);
    } else if (target === "effect") {
      if (!("setEffectStyleIdAsync" in bindNode2)) throw new Error("Node does not support effect styles");
      await (bindNode2 as FrameNode | RectangleNode).setEffectStyleIdAsync(payload.styleId);
    } else {
      throw new Error("Unknown style bind target: " + target);
    }

    // Look up the style name for the response
    var boundStyleName: string | undefined;
    try {
      if (target === "text") {
        var tstyles = await figma.getLocalTextStylesAsync();
        var tsm = tstyles.find(function(s) { return s.id === payload.styleId; });
        if (tsm) boundStyleName = tsm.name;
      } else if (target === "effect") {
        var estyles = await figma.getLocalEffectStylesAsync();
        var esm = estyles.find(function(s) { return s.id === payload.styleId; });
        if (esm) boundStyleName = esm.name;
      } else {
        var pstyles = await figma.getLocalPaintStylesAsync();
        var psm = pstyles.find(function(s) { return s.id === payload.styleId; });
        if (psm) boundStyleName = psm.name;
      }
  ...
  if (command.type === "createDesignSystemStyles") {
    var dssResults: Record<string, unknown> = {};
    var dssCreated = 0;
    var dssUpdated = 0;
    var dssSkipped = 0;
    var dssFailed = 0;
    ...
    // Paint styles
    var paintResults: Record<string, unknown>[] = [];
    if (Array.isArray(payload.paintStyles)) {
      for (var psi = 0; psi < (payload.paintStyles as Record<string, unknown>[]).length; psi += 1) {
        try {
          var psPayload = Object.assign({}, (payload.paintStyles as Record<string, unknown>[])[psi], { upsert: payload.upsert !== false });
          var psRes = await createOrUpdatePaintStyle(psPayload);
          paintResults.push(psRes);
          if (psRes.operation === "created") dssCreated += 1;
          else if (psRes.operation === "updated") dssUpdated += 1;
          else if (psRes.operation === "skipped") dssSkipped += 1;
        } catch (_psError) {
          dssFailed += 1;
    ...
    // Text styles
    var textResults: Record<string, unknown>[] = [];
    if (Array.isArray(payload.textStyles)) {
      for (var tsi = 0; tsi < (payload.textStyles as Record<string, unknown>[]).length; tsi += 1) {
        try {
          var tsPayload = Object.assign({}, (payload.textStyles as Record<string, unknown>[])[tsi], { upsert: payload.upsert !== false });
          var tsRes = await createOrUpdateTextStyle(tsPayload);
          textResults.push(tsRes);
          if (tsRes.operation === "created") dssCreated += 1;
          else if (tsRes.operation === "updated") dssUpdated += 1;
          else if (tsRes.operation === "skipped") dssSkipped += 1;
        } catch (_tsError) {
          dssFailed += 1;
    ...
    // Effect styles
    var effectResults: Record<string, unknown>[] = [];
    if (Array.isArray(payload.effectStyles)) {
      for (var esi = 0; esi < (payload.effectStyles as Record<string, unknown>[]).length; esi += 1) {
        try {
          var esPayload = Object.assign({}, (payload.effectStyles as Record<string, unknown>[])[esi], { upsert: payload.upsert !== false });
          var esRes = await createOrUpdateEffectStyle(esPayload);
          effectResults.push(esRes);
          if (esRes.operation === "created") dssCreated += 1;
          else if (esRes.operation === "updated") dssUpdated += 1;
          else if (esRes.operation === "skipped") dssSkipped += 1;
        } catch (_esError) {
          dssFailed += 1;
  ...
  if (command.type === "auditStyles") {
    ...
    for (var pnk = 0; pnk < pnKeys.length; pnk += 1) { if (paintNames[pnKeys[pnk]] > 1) duplicates.push(pnKeys[pnk] + " (paint x" + paintNames[pnKeys[pnk]] + ")"); }
    var tnKeys = Object.keys(textNames);
    for (var tnk = 0; tnk < tnKeys.length; tnk += 1) { if (textNames[tnKeys[tnk]] > 1) duplicates.push(tnKeys[tnk] + " (text x" + textNames[tnKeys[tnk]] + ")"); }
    var enKeys = Object.keys(effectNames);
    for (var enk = 0; enk < enKeys.length; enk += 1) { if (effectNames[enKeys[enk]] > 1) duplicates.push(enKeys[enk] + " (effect x" + effectNames[enKeys[enk]] + ")"); }

    // Expected vs missing
    var missingExpected: string[] = [];
    if (payload.expected && typeof payload.expected === "object") {
      var exp = payload.expected as Record<string, unknown>;
      if (Array.isArray(exp.paintStyles)) {
        var expPaint = exp.paintStyles as string[];
        for (var epi = 0; epi < expPaint.length; epi += 1) {
          if (!paintNames[expPaint[epi]]) missingExpected.push(expPaint[epi] + " (paint)");
        }
      }
      if (Array.isArray(exp.textStyles)) {
        var expText = exp.textStyles as string[];
        for (var eti = 0; eti < expText.length; eti += 1) {
          if (!textNames[expText[eti]]) missingExpected.push(expText[eti] + " (text)");
        }
      }
      if (Array.isArray(exp.effectStyles)) {
        var expEffect = exp.effectStyles as string[];
        for (var eei = 0; eei < expEffect.length; eei += 1) {
          if (!effectNames[expEffect[eei]]) missingExpected.push(expEffect[eei] + " (effect)");
        }
      }
    }
  ...
  if (command.type === "auditNodeStyleBinding") {
    ...
    if (!auditRootNode || auditRootNode.type === "DOCUMENT" || auditRootNode.type === "PAGE") throw new Error("Scene node not found");

    var expectedPrefix = typeof payload.expectedPrefix === "string" ? payload.expectedPrefix : "";
    var checkedNodes = 0;
    var styleBindings: Record<string, unknown>[] = [];
    var unboundStyledNodes: Record<string, unknown>[] = [];

    function walkNodeStyleAudit(node: BaseNode, currentDepth: number) {
      ...
      if ("fillStyleId" in sceneNode) {
        ...
        } else if ("fills" in sceneNode && sceneNode.fills !== figma.mixed && (sceneNode.fills as Paint[]).length > 0) {
      ...
      if (sceneNode.type === "TEXT") {
        var tsi2 = (sceneNode as TextNode).textStyleId;
      ...
      if ("children" in sceneNode && currentDepth > 0) {
        var children = (sceneNode as FrameNode | GroupNode | ComponentNode | InstanceNode).children;
    ...
    var wrongPrefixBindings: Record<string, unknown>[] = [];
    if (expectedPrefix) {
